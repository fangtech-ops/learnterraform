...
resource "github_repository_file" "cm" {
  ...
  content = templatefile(
    "${path.module}/files/templates/configmap.tpl",
    {
      ...
      route53_public_hosted_zone1_domain_name_list = "{${data.terraform_remote_state.networking_infra.outputs.domain_name_qa71_narvarcorp_net}}"
      route53_public_hosted_zone1_id               = data.terraform_remote_state.networking_infra.outputs.zone_id_qa71_narvarcorp_net

      irsa_cert_manager_role_arn       = data.terraform_remote_state.networking_irsa.outputs.narvar-irsa-role-for-cert-manager-eks-qa71-iam_role_arn
      irsa_external_dns_role_arn       = data.terraform_remote_state.networking_irsa.outputs.narvar-irsa-role-for-external-dns-eks-qa71-iam_role_arn
